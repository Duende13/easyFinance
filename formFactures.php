...
if(isset($_GET['id']) && $_GET['id']){
	$id = $_GET['id'];
	$selectFactures = mysqli_query($db,"SELECT * FROM `factures".ucfirst($type)."` WHERE `id`='$id'") or trigger_error(mysqli_error($db),E_USER_ERROR);
	$f = mysqli_fetch_array($selectFactures);
	extract($f);
...
if (!isset($_GET['ajaxed'])) {
	$myInterface->set_title("EasyFinance – Factura ".$typeSingulier);
