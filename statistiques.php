...
$selectFacturesSortantes = mysqli_query($db,"SELECT * FROM facturesSortantes ".$req." ORDER BY ".$req2." date") or trigger_error(mysqli_error($db),E_USER_ERROR);
$nombreFacturesSortantes = mysqli_num_rows($selectFacturesSortantes);
$selectFacturesEntrantes = mysqli_query($db,"SELECT * FROM facturesEntrantes ".$req." ORDER BY ".$req2." date") or trigger_error(mysqli_error($db),E_USER_ERROR);
?>
<?php
$myInterface->set_title("EasyFinance – Statistiques");
...
<?php
while($f = mysqli_fetch_array($selectFacturesSortantes)){
...
while($f = mysqli_fetch_array($selectFacturesEntrantes)){
	if($f['date'] < $annee."-04-00"){
