...
$select_trimestres = mysqli_query("SELECT * FROM trimestres WHERE type='sortantes' ORDER BY `annee`, `trimestre`") or trigger_error(mysqli_error($db),E_USER_ERROR);
$select_first_trimestres = mysqli_query("SELECT * FROM trimestres WHERE type='sortantes' ORDER BY `annee`, `trimestre` LIMIT 1") or trigger_error(mysqli_error($db),E_USER_ERROR);
$select_trimestres_d = mysqli_query("SELECT * FROM trimestres WHERE type='entrantes' ORDER BY `annee`, `trimestre`") or trigger_error(mysqli_error($db),E_USER_ERROR);
$first = mysqli_fetch_array($select_first_trimestres);
