...
$selectContrats = mysqli_query("SELECT * FROM `".$table."` LEFT JOIN `clients` ON `".$table."`.`id_client`=`clients`.`id_client` ".$req." ORDER BY ".$req2." `date`, `numero`") or trigger_error(mysqli_error($db),E_USER_ERROR);
$nombreContrats = mysqli_num_rows($selectContrats);
?>
...
<?php
$client_count=0;
while ($f = mysqli_fetch_array($selectContrats)) {
	if (!$ordre) {
		// trimestre
