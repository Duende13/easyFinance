<?php
  ...
  include('include/config.php');
	...
	$sql = 'select count(*) FROM budget_services where   budget_id =' .$id;
	$result = mysqli_query($db, $sql)  or die ('Erreur SQL : ' .$sql .mysqli_connect_error() );
    ...
    // $pdf->Cell( 160, 270, $num_page . '/' . $nb_page, 0, 0, 'C');

		// facture, date echeance et reglement et obs
		$select = 'select created_at,invoice_number,total,vat,nett,auto_id,subtotal FROM budgets where id =' .$id;
		$result = mysqli_query($db, $select)  or die ('Erreur SQL : ' .$select .mysqli_connect_error() );
		$row = mysqli_fetch_row($result);
		mysqli_free_result($result);

		//Numero de factura
		$num_fact = "FACTURA: " . str_pad($row[1], 6, '0', STR_PAD_LEFT);
		$pdf->SetLineWidth(0); $pdf->SetFillColor(192); $pdf->Rect(120, 65, 85, 15, 8, "DF");
		$pdf->SetXY( 120, 65 ); $pdf->SetFont( "Arial", "B", 12 ); $pdf->Cell( 120, 8, $num_fact, 0, 0, 'C');

		// nom du fichier final TODO: NUMERO DE ARCHIVO IGUAL QUE LA FACTURA?
		$nom_file = "fact_" . $row[1] .'-' . str_pad($row[1], 4, '0', STR_PAD_LEFT) . ".pdf";
