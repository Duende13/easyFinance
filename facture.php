...
if(isset($_GET['id']) && !empty($_GET['id'])){
	$id = $_GET['id'];
	$selectFactureSortante = mysqli_query($db,"SELECT * FROM `facturesSortantes` LEFT JOIN `clients` ON `facturesSortantes`.`id_client`=`clients`.`id_client` WHERE `id`='$id'") or trigger_error(mysqli_error($db),E_USER_ERROR);
} else {
	header("location:./");
}
$f = mysqli_fetch_array($selectFactureSortante);
extract($f);

$selectUtilisateur = mysqli_query($db,"SELECT * FROM `utilisateur` WHERE `id`='$id_usr'") or trigger_error(mysqli_error($db),E_USER_ERROR);
$utilisateur = mysqli_fetch_array($selectUtilisateur);
$selectClients = mysqli_query($db,"SELECT * FROM clients") or trigger_error(mysqli_error($db),E_USER_ERROR);
if($tva){
	$tva = "TVA : ".$tva;
}
