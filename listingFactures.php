...
$client_count=0;
if ($type == 'entrantes') {
	while($f = mysqli_fetch_array($selectFactures)){
...
$selectTrimestre = mysqli_query($db,"SELECT * FROM `trimestres` WHERE `annee`='".substr($f['date'],0,4)."' AND `trimestre`='$key_trimestre' AND `type`='$type'") or trigger_error(mysqli_error($db),E_USER_ERROR);;
$totauxTrim = mysqli_fetch_array($selectTrimestre);
if($totauxTrim['id']){
	if($totauxTrim['montant_htva'] != $tt_htva){
		$idTrim = $totauxTrim['id'];
		$updateTrimestre = mysqli_query($db,"UPDATE `trimestres` SET `montant_htva`='$tt_htva', `montant_tva`='$tt_tva', `montant_tvac`='$tt_tvac' WHERE `id`='$idTrim' AND `type`='$type'") or trigger_error(mysqli_error($db),E_USER_ERROR);;
	}
}else{
	$insert_trimestre = mysqli_query($db,"INSERT INTO `trimestres` (`annee`,`trimestre`,`montant_htva`,`montant_tva`,`montant_tvac`,`type`) VALUES ('".substr($f['date'],0,4)."','$key_trimestre','$tt_htva','$tt_tva','$tt_tvac','$type')") or trigger_error(mysqli_error($db),E_USER_ERROR);;
}
